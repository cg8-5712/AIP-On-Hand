...
fn query_procedure_legs(
    ...
    let rows = statement.query_map(params![procedure_id], |row| {
        Ok((
            ...
            lat_lon_from_optional(row.get::<_, Option<f64>>(5)?, row.get::<_, Option<f64>>(6)?),
            row.get::<_, Option<String>>(7)?,
            row.get::<_, Option<String>>(8)?,
            lat_lon_from_optional(
...
fn query_transition_legs(
    ...
    let rows = statement.query_map(params![transition_id], |row| {
        Ok((
            ...
            row.get::<_, Option<String>>(2)?,
            row.get::<_, Option<String>>(3)?,
            lat_lon_from_optional(row.get::<_, Option<f64>>(4)?, row.get::<_, Option<f64>>(5)?),
            row.get::<_, Option<String>>(6)?,
            row.get::<_, Option<String>>(7)?,
            lat_lon_from_optional(row.get::<_, Option<f64>>(8)?, row.get::<_, Option<f64>>(9)?),
        ))
    })?;
...
fn resolve_procedure_leg_point(
    ...
    let ident = normalized_optional_string(fix_ident.as_deref())
        .map(str::to_string)
        .or_else(|| {
            normalized_optional_string(recommended_fix_ident.as_deref()).map(str::to_string)
        });
...
fn resolve_nav_ident_position(
    ...
    let Some(ident) = normalized_optional_string(ident) else {
        return Ok(None);
    };
    let ident_key = normalized_ident_key(ident);
...
fn select_nav_ident_candidate(
    ...
    if let Some(airport_ident) = normalized_optional_string(airport_ident) {
    ...
    if let Some(region) = normalized_optional_string(region) {
...
fn normalized_ident_key(value: &str) -> String {
