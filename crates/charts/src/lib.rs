use aip_domain::{EaipAirportChartsResponse, EaipChartScope, EaipChartSummary, EaipStatusResponse};
...
impl EaipChartService {
    pub fn from_env() -> Self {
        ...
        let disabled = |configured: bool, message: String, package_file: Option<String>| Self {
        ...
        let Some(password) = password else {
            let message =
                "eAIP package is disabled because AIP_EAIP_PACKAGE_PASSWORD is not configured."
                    .to_string();
            warn!("{message}");
            return disabled(false, message, package_file);
        };

        match Self::load_ready_store(PathBuf::from(&package_path), &password, cache_capacity) {
            Ok(store) => {
                info!(
                    ...
                    store.status.chart_count,
                    store.status.airport_count
                );
                Self {
            ...
            Err(error) => {
                ...
                warn!("{message}");
                disabled(true, message, package_file)
            }
        }
    }
        ...
        reader
            ...
            .map_err(|error| {
                ChartError::Internal(format!(
                    "failed to read chart `{}` from encrypted package: {error}",
    ...
    fn load_ready_store(
        ...
        Ok(ReadyStore {
            ...
            status: EaipStatusResponse {
                configured: true,
                ready: true,
                package_file: package_file_name(&package_path),
                cycle: Some(cycle),
                chart_count,
                airport_count,
...
fn collect_chart_records(reader: &PackageReader) -> Vec<ChartRecord> {
    let mut paths = reader.index.lookup.keys().cloned().collect::<Vec<_>>();
    paths.sort();
